"""Locust user definition for InferenceBenchmarker waves.
...
def on_quitting(environment, **kwargs):
    if _SAMPLE_HW:
        ...
        if _cpu_samples and _mem_samples:
            ...
            # Write hw metrics to file for programmatic reading (_burst.py alternative path).
            # Enabled when BENCHMARKER_SAMPLE_HW=1 — file lands in requests_fired/hw_metrics.json.
            # import json
            # wave_dir = os.environ.get('LOCUST_WAVE_DIR', '')
            # if wave_dir:
            #     hw_path = os.path.join(wave_dir, 'hw_metrics.json')
            #     with open(hw_path, 'w') as _f:
            #         json.dump({
            #             'cpu_max': cpu_max, 'cpu_avg': cpu_avg,
            #             'memory_max': mem_max, 'memory_avg': mem_avg,
            #             'cpu_count': cpu_count, 'total_ram_gb': total_ram_gb,
            #             'sample_count': len(_cpu_samples),
            #         }, _f)
...
class InferenceBenchmarker(FastHttpUser):
    ...
    # Each user fires one request then sleeps indefinitely via wait_time,
    # keeping the user alive so Locust doesn't respawn it to fill the --users target.
    # Locust calls wait_time after the task returns; inf means it never runs again.
    wait_time = constant(float('inf'))

    @task
    def invoke_endpoint(self):
        _invoke()
        # gevent.sleep(float('inf'))  # alternative to wait_time=constant(inf)
